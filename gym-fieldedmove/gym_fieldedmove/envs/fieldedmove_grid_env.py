...
class FieldedMoveGrid(gym.Env):
	...
	def reset(self):
		...
		self.position = np.random.randint(low=self.agent_size, high=self.window_width - self.goal_size, size=2).astype(np.float64)
		self._reset_field()
		self._reset_task()
		# if self.mode == "human":
	...
	def _reset_task(self):
		...
		# self.goal += self.center
		self.goal = np.random.randint(low=self.goal_size, high=self.window_width-self.goal_size, size=2).astype(np.float64)
	...
	def step(self, target):
		if self.done == 1:
			...
			self.position = np.clip(self.position, 0, self.env_height-1) # bounding
			...
			# if self.count % 10 == 0 and self.field_change:
			# 	self._reset_field()
