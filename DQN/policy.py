...
class QNetwork(Network):
    def __init__(self, ob_space, ac_space, hidsize,
                 ob_mean, ob_std, feat_dim, layernormalize, nl,
                 n_env, n_steps, reuse, n_lstm=256, scope="policy"):
        super(QNetwork, self).__init__(ob_space, ac_space, hidsize,
                            ob_mean, ob_std, feat_dim, layernormalize, nl,
        ...
        with tf.variable_scope(scope, reuse=self.reuse):
            ...
            # x = tf.concat([x, rnn_output], axis=1)
            x = fc(x, units=hidsize, activation=activ, name="fc1")
            self.q = fc(x, units=get_action_n(self.ac_space), activation=activ, name="q")
            # x = fc(x, units=hidsize, activation=activ, name="fc2")
            # rnn_output = q

            # pdparam, vpred = self.get_pdparam(x)
    ...
    def get_ac_qval(self, ob, state=None, mask=None):
        a, q = \
            getsess().run([self.q_action, self.q],
